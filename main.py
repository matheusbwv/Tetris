...
FONT_PATH = os.path.join('assets', 'fonts', 'BigBlueTermPlusNerdFont-Regular.ttf') 
...
def draw_window(surface, grid, current_score, high_score):
    ...
    label_player_score = font_scores.render(f'P = {current_score}', 1, (255,255,255))
    ...
    label_game_record = font_scores.render(f'GR = {high_score}', 1, (255,255,255))
    ...
    for i in range(len(grid)):
        ...
        for j in range(len(grid[i])):
            pygame.draw.line(surface, (128, 128, 128), (top_left_x + j*block_size, top_left_y),(top_left_x + j*block_size, top_left_y + play_height), 1)
...
def draw_game_over_screen(surface, final_score, high_score):
    ...
    rect_game_over = label_game_over.get_rect(center=(s_width / 2, s_height / 2 - 100))
    surface.blit(label_game_over, rect_game_over)

    # Pontuação Atual
    label_final_score = font_scores.render(f'Sua Pontuação: {final_score}', True, (255, 255, 255))
    rect_final_score = label_final_score.get_rect(center=(s_width / 2, s_height / 2 - 20))
    surface.blit(label_final_score, rect_final_score)

    # High Score
    label_high_score = font_scores.render(f'Recorde: {high_score}', True, (255, 255, 255))
    rect_high_score = label_high_score.get_rect(center=(s_width / 2, s_height / 2 + 40))
    surface.blit(label_high_score, rect_high_score)

    # Prompt para o usuário
    label_prompt = font_prompt.render('Pressione ESPAÇO para jogar novamente ou ESC para o menu', True, (200, 200, 200))
    rect_prompt = label_prompt.get_rect(center=(s_width / 2, s_height / 2 + 120))
...
def main(win):
    ...
    while run:
                    ...
                    if event.key == pygame.K_LEFT:
                        ...
                        for dx, dy in WALL_KICKS:
                            ...
                            if valid_space(current_piece, grid, locked_positions): 
                                ...
                                test_piece_below = Piece(current_piece.x, current_piece.y + 1, current_piece.shape)
        ...
        if not paused:
            ...
            test_piece_below = Piece(current_piece.x, current_piece.y + 1, current_piece.shape)
            test_piece_below.rotation = current_piece.rotation
            can_fall_naturally = valid_space(test_piece_below, grid, locked_positions) 
            ...
            if check_lost(locked_positions):
                ...
                action = draw_game_over_screen(win, score, current_high_score_after_game)
